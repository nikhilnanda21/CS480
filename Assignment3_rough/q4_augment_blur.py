...
for blur in blur_list:
    ...
    for (i, X_data) in enumerate(x_dummy_test):
      ...
      X_blur_test.append(np.array(img_blur).reshape(32, 32, 1))

    X_blur_test = np.array(X_blur_test)

print(X_blur.shape)
print(X_blur_test.shape)
...
for i in range(epoch):
  train_score = model.fit(X_blur, y_train,
            batch_size=32,
            validation_split=0.2,
            initial_epoch=i,
            epochs=i+1,
            verbose=1)
  test_score = model.evaluate(X_blur_test, y_test, verbose=1)
...
# Storing the model
model.save('./cnn/model2.h5')
with open('./data/train_2.pkl', 'wb') as file_history:
  pickle.dump(train_score_list, file_history)

# Storing the results from training the model
with open('./data/test_2.pkl', 'wb') as file_history:
