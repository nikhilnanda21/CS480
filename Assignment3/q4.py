...
# FULLY CONNECTED LAYER - 4096x2, 10x1
model.add(Flatten())
...
for i in range(epoch):
  train_score = model.fit(x_dummy_train, y_train,
            batch_size=32,
            validation_split=0.2,
            initial_epoch=i,
            epochs=i+1,
            verbose=1)
  test_score = model.evaluate(x_dummy_test, y_test, verbose=1)
...
# Storing the model
model.save('./cnn/model1.h5')
with open('./data/train.pkl', 'wb') as file_history:
  pickle.dump(train_score_list, file_history)

# Storing the results from training the model
with open('./data/test.pkl', 'wb') as file_history:
